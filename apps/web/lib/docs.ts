...
/** Root docs/ lives two levels up from this app (apps/web -> repo root) and is
 *  the single source of truth for documentation content. Every docs route is
 *  statically generated (SSG, `dynamicParams = false`), so these reads run only
 *  at build time and the rendered HTML is baked into the output — the server
 *  bundle never touches the filesystem at runtime. (Turbopack emits one NFT
 *  trace warning because this path escapes the app root; it is expected and
 *  harmless for fully-static routes.) */
