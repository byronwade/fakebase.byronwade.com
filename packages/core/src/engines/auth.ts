...
export interface AuthAdminApi {
  ...
  getUserById(id: string): AuthResult<{ user: AuthUser }>;
  inviteUserByEmail(email: string): AuthResult<{ user: null; messageId: string }>;
  generateLink(opts: { type: string; email: string }): AuthResult<{
...
export interface AuthEngine {
  signUp(params: {
    ...
    | { data: { user: AuthUser; session: null; needsConfirmation: true }; error: null }
    | { data: { user: null; session: null }; error: { message: string } }
  >;

  signInWithPassword(
    params: { email: string; password: string } | { phone: string; password: string },
  ): Promise<
    | { data: { user: AuthUser; session: AuthSession }; error: null }
    | { data: { user: null; session: null }; error: { message: string } }
  >;
  ...
  verifyOtp(
    params:
      | { email: string; token: string; type: OtpRecord["type"] }
      | { phone: string; token: string; type: OtpRecord["type"] },
  ...
  setSession(params: {
    access_token: string;
    refresh_token: string;
  }): Promise<AuthResult<{ session: AuthSession }>>;
  exchangeCodeForSession(code: string): Promise<AuthResult<{ session: AuthSession }>>;
  signOut(options?: {
