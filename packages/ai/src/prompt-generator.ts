...
- **Local adapter:** \`@fakebase/adapter-json\` (file-backed, stored in \`.fakebase/\`)
- **Auth:** \`@fakebase/auth\` (in-memory sessions, no real JWT verification)
- **Storage:** \`@fakebase/storage\` (local filesystem or memory)
- **Realtime:** \`@fakebase/realtime\` (in-process pub/sub, not WebSocket)
