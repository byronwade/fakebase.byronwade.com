...
function objectKey(bucketId: string, path: string): string {
...
export class LocalStorageService implements StorageEngine {
  ...
  constructor(
    ...
    private readonly objects: Map<string, ObjectRecord>,
    private readonly signedUrls: Map<string, SignedUrlRecord>,
  ...
  getBucket(id: string): StorageResult<BucketRecord> {
    ...
    if (!bucket) return makeError("Bucket not found", "404");
    return ok(bucket);
  }
  ...
  async emptyBucket(id: string): Promise<StorageResult<{ message: string }>> {
    if (!this.buckets.has(id)) return makeError("Bucket not found", "404");
  ...
  from(bucketId: string): StorageBucketApi {
    ...
    return {
      ...
      async move(
        ...
        if (!existing) return makeError("Object not found", "404");
      ...
      async remove(paths: string[]): Promise<StorageResult<ObjectRecord[]>> {
        const removed: ObjectRecord[] = [];
      ...
      list(prefix?: string, options: ListOptions = {}): StorageResult<ListEntry[]> {
        ...
        for (const obj of svc.objects.values()) {
          if (obj.bucket_id !== bucketId) continue;
          ...
          if (search && !relName.includes(search)) continue;
          ...
          if (slashIdx !== -1) {
            ...
            const fileObj: FileObject = { ...obj, name: relName };
        ...
        if (sortBy) {
          entries.sort((a, b) => {
            const col = sortBy.column as keyof ListEntry;
      ...
      async download(
        ...
        const key = objectKey(bucketId, path);
        const obj = svc.objects.get(key);
        if (!obj) return makeError("Object not found", "404");
      ...
      async createSignedUrl(
        ...
        if (!svc.buckets.has(bucketId)) return makeError("Bucket not found", "404");
