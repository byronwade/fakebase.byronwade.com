...
type TestDatabase = {
  public: {
    Tables: {
      products: {
        Row: { id: string; name: string; price: number; in_stock: boolean };
        Insert: { id?: string; name: string; price: number; in_stock?: boolean };
        Update: { id?: string; name?: string; price?: number; in_stock?: boolean };
      };
      orders: {
        Row: { id: string; product_id: string; quantity: number; total: number };
        Insert: { id?: string; product_id: string; quantity: number; total: number };
        Update: { id?: string; product_id?: string; quantity?: number; total?: number };
      };
    };
...
function makeClient() {
  ...
  const client = createClient<TestDatabase>("http://localhost", "anon-key", {
    kernel,
  });
...
describe("createClient", () => {
  ...
  it("ignores the url and key parameters", () => {
    const kernel = createMemoryKernel(ir);
    expect(() =>
      createClient("irrelevant-url", "irrelevant-key", { kernel }),
    ).not.toThrow();
  });
});
...
describe("client.auth", () => {
  ...
  it("onAuthStateChange fires INITIAL_SESSION callback", async () => {
    ...
    const {
      ...
      if (event === "INITIAL_SESSION") received = true;
    });
...
describe("client.functions.invoke()", () => {
  ...
  it("calls registered handler and returns result", async () => {
    ...
    ).register("hello", async () => ({ message: "Hello!" }));
    const { data, error } = await client.functions.invoke("hello");
    expect(error).toBeNull();
    expect((data as Record<string, unknown>)?.["message"]).toBe("Hello!");
  });
});
