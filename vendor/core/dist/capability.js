...
export class CapabilityRegistry {
    ...
    assertSupported(name) {
        ...
        if (entry.status === CapabilityStatus.STUB ||
            entry.status === CapabilityStatus.UNSUPPORTED) {
            throw new CapabilityError(name, `'${name}' is ${entry.status.toLowerCase()} on this adapter.${entry.notes ? ` ${entry.notes}` : ""}`, entry.docsLink);
        }
    }
    /** Render a markdown compatibility table for documentation or CLI output. */
    toMarkdown() {
        const rows = this.getAll()
            .map((e) => `| ${e.name} | ${e.status} | ${e.notes ?? ""} | ${e.docsLink ? `[docs](${e.docsLink})` : ""} |`)
            .join("\n");
        return [
