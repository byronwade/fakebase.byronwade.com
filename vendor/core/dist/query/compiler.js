...
function errorResult(err) {
    const fbErr = err instanceof FakebaseError
        ? err
        : new FakebaseError(FakebaseErrorCode.ADAPTER_ERROR, err instanceof Error ? err.message : String(err));
    return {
        data: null,
        error: fbErr,
        count: null,
 ...
 * adapters.
 */
export function compileQuery(options, rows) {
    try {
        // Apply filters
        let filtered = rows;
        if (options.filters.length > 0) {
            filtered = rows.filter((row) => options.filters.every((f) => applyFilter(row, f)));
        }
        ...
        if (options.orderBy && options.orderBy.length > 0) {
            ...
            filtered = [...filtered].sort((a, b) => {
                for (const ord of orderings) {
                    const diff = compareValues(a[ord.column], b[ord.column], ord.ascending, ord.nullsFirst ?? false);
                    if (diff !== 0)
                        return diff;
                }
                return 0;
        ...
        if (options.select && options.select.length > 0) {
            data = filtered.map((row) => projectRow(row, options.select));
        }
        else {
            data = filtered;
        }
