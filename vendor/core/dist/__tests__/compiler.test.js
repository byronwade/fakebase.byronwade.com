...
describe("compileQuery – filtering", () => {
    it("filters by eq", () => {
        const result = compileQuery(baseOpts({ filters: [{ column: "active", operator: "eq", value: true }] }), rows);
        expect(result.data).toHaveLength(3);
        expect(result.data?.every((r) => r["active"] === true)).toBe(true);
    });
    it("filters by gt", () => {
        const result = compileQuery(baseOpts({ filters: [{ column: "age", operator: "gt", value: 28 }] }), rows);
        expect(result.data?.map((r) => r["name"])).toEqual(["Alice", "Charlie"]);
    });
    it("multiple filters are ANDed", () => {
        const result = compileQuery(baseOpts({
            filters: [
    ...
    it("returns empty when no rows match", () => {
        const result = compileQuery(baseOpts({ filters: [{ column: "age", operator: "gt", value: 100 }] }), rows);
        expect(result.data).toHaveLength(0);
    });
});
describe("compileQuery – ordering", () => {
    it("sorts ascending", () => {
        const result = compileQuery(baseOpts({ orderBy: [{ column: "age", ascending: true }] }), rows);
        const ages = result.data?.map((r) => r["age"]);
        expect(ages).toEqual([22, 25, 28, 30, 35]);
    });
    it("sorts descending", () => {
        const result = compileQuery(baseOpts({ orderBy: [{ column: "age", ascending: false }] }), rows);
        const ages = result.data?.map((r) => r["age"]);
