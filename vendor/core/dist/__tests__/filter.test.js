...
describe("applyFilter – leaf operators", () => {
    it("eq", () => {
        expect(applyFilter(row, { column: "name", operator: "eq", value: "Alice" })).toBe(true);
        expect(applyFilter(row, { column: "name", operator: "eq", value: "Bob" })).toBe(false);
    });
    it("neq", () => {
        expect(applyFilter(row, { column: "name", operator: "neq", value: "Bob" })).toBe(true);
        expect(applyFilter(row, { column: "name", operator: "neq", value: "Alice" })).toBe(false);
    });
    it("gt / gte", () => {
    ...
    it("like", () => {
        expect(applyFilter(row, { column: "name", operator: "like", value: "Al%" })).toBe(true);
        expect(applyFilter(row, { column: "name", operator: "like", value: "Bob%" })).toBe(false);
        expect(applyFilter(row, { column: "name", operator: "like", value: "%ice" })).toBe(true);
    });
    it("ilike – case-insensitive", () => {
        expect(applyFilter(row, { column: "name", operator: "ilike", value: "ALICE" })).toBe(true);
        expect(applyFilter(row, { column: "name", operator: "ilike", value: "al%" })).toBe(true);
    });
    it("is null / not null", () => {
        expect(applyFilter(row, { column: "parent_id", operator: "is", value: null })).toBe(true);
        expect(applyFilter(row, { column: "name", operator: "is", value: null })).toBe(false);
    });
    it("is true / false", () => {
        expect(applyFilter(row, { column: "active", operator: "is", value: true })).toBe(true);
        expect(applyFilter(row, { column: "active", operator: "is", value: false })).toBe(false);
    });
    it("in", () => {
        expect(applyFilter(row, { column: "age", operator: "in", value: [20, 30, 40] })).toBe(true);
        expect(applyFilter(row, { column: "age", operator: "in", value: [20, 25] })).toBe(false);
    });
    it("contains – array", () => {
        expect(applyFilter(row, { column: "tags", operator: "contains", value: ["admin"] })).toBe(true);
        expect(applyFilter(row, {
            column: "tags",
            operator: "contains",
            value: ["admin", "user"],
        })).toBe(true);
        expect(applyFilter(row, { column: "tags", operator: "contains", value: ["superuser"] })).toBe(false);
    });
    ...
    it("containedBy", () => {
        ...
        expect(applyFilter(row, { column: "tags", operator: "containedBy", value: ["admin"] })).toBe(false);
    });
    it("overlaps", () => {
        expect(applyFilter(row, {
            column: "tags",
        ...
        expect(applyFilter(row, { column: "tags", operator: "overlaps", value: ["superuser"] })).toBe(false);
    });
    it("match – full-text substring", () => {
        expect(applyFilter(row, { column: "name", operator: "match", value: "lic" })).toBe(true);
        expect(applyFilter(row, { column: "name", operator: "match", value: "xyz" })).toBe(false);
    });
    it("not – negation", () => {
        expect(applyFilter(row, { column: "name", operator: "not", value: "Bob" })).toBe(true);
        expect(applyFilter(row, { column: "name", operator: "not", value: "Alice" })).toBe(false);
    });
});
