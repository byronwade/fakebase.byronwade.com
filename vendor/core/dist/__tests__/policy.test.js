...
describe("PolicyEngine – RLS disabled", () => {
    ...
    it("allows all writes when RLS is off", () => {
        ...
        const engine = new PolicyEngine(registry);
        const ctx = { role: "anon" };
        expect(engine.evaluateWrite("posts", "public", { id: "1" }, ctx, "INSERT")).toBe(true);
    });
});
describe("PolicyEngine – service_role bypass", () => {
    ...
    it("service_role bypasses RLS writes", () => {
        ...
        const engine = new PolicyEngine(registry);
        const ctx = { role: "service_role" };
        expect(engine.evaluateWrite("posts", "public", { id: "1" }, ctx, "DELETE")).toBe(true);
    });
});
describe("PolicyEngine – default deny", () => {
    it("denies reads when RLS enabled but no policies", () => {
        ...
        const registry = new SchemaRegistry(makeSchema(table));
        const engine = new PolicyEngine(registry);
        const ctx = { role: "authenticated", userId: "user-1" };
    ...
    it("denies writes when RLS enabled but no policies", () => {
        ...
        const engine = new PolicyEngine(registry);
        const ctx = { role: "authenticated", userId: "user-1" };
        expect(engine.evaluateWrite("posts", "public", { id: "1" }, ctx, "INSERT")).toBe(false);
    });
});
describe("PolicyEngine – auth.uid() policy", () => {
    ...
    it("allows read for own row", () => {
        const ctx = { role: "authenticated", userId: "user-1" };
        expect(engine.evaluateRead("posts", "public", { user_id: "user-1" }, ctx)).toBe(true);
    });
    it("denies read for other's row", () => {
        const ctx = { role: "authenticated", userId: "user-2" };
        expect(engine.evaluateRead("posts", "public", { user_id: "user-1" }, ctx)).toBe(false);
    });
    it("allows insert for own row", () => {
        const ctx = { role: "authenticated", userId: "user-1" };
        expect(engine.evaluateWrite("posts", "public", { user_id: "user-1" }, ctx, "INSERT")).toBe(true);
    });
    it("denies insert for other's row", () => {
        const ctx = { role: "authenticated", userId: "user-2" };
        expect(engine.evaluateWrite("posts", "public", { user_id: "user-1" }, ctx, "INSERT")).toBe(false);
    });
