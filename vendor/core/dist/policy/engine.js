 ...
 */
function evalExpression(expr, row, ctx) {
    ...
    console.warn(`[fakebase/policy] Cannot evaluate expression: "${expr}". Defaulting to ALLOW. ` +
...
function policyApplies(policy, role, op) {
...
export class PolicyEngine {
     ...
     */
    evaluateRead(table, schema, row, ctx) {
        if (ctx.role === "service_role")
            return true;
        const tableIR = this.registry.getTable(schema, table);
        if (!tableIR || !tableIR.rlsEnabled)
            return true;
        const applicable = tableIR.policies.filter((p) => policyApplies(p, ctx.role, "SELECT"));
        if (applicable.length === 0)
            return false; // default deny
        const permissive = applicable.filter((p) => p.permissive);
        const restrictive = applicable.filter((p) => !p.permissive);
        const allowedByPermissive = permissive.length === 0
            ? false
            : permissive.some((p) => (p.using ? evalExpression(p.using, row, ctx) : true));
        if (!allowedByPermissive)
            return false;
        const deniedByRestrictive = restrictive.some((p) => p.using && !evalExpression(p.using, row, ctx));
     ...
     */
    evaluateWrite(table, schema, row, ctx, op) {
        if (ctx.role === "service_role")
            return true;
        const tableIR = this.registry.getTable(schema, table);
        if (!tableIR || !tableIR.rlsEnabled)
        ...
        const allowedByPermissive = permissive.length === 0 ? false : permissive.some(checkExpr);
        if (!allowedByPermissive)
            return false;
