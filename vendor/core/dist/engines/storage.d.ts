...
export interface StorageBucketApi {
    upload(path: string, body: Uint8Array | string | Blob, options?: UploadOptions): Promise<StorageResult<{
        id: string;
        path: string;
        fullPath: string;
    }>>;
    update(path: string, body: Uint8Array | string | Blob, options?: UploadOptions): Promise<StorageResult<{
        id: string;
        path: string;
        fullPath: string;
    }>>;
    ...
    getPublicUrl(path: string, options?: GetPublicUrlOptions): {
        data: {
            publicUrl: string;
        };
    };
    createSignedUrl(path: string, expiresIn: number, options?: CreateSignedUrlOptions): Promise<StorageResult<{
        signedUrl: string;
        token: string;
        path: string;
    }>>;
    createSignedUrls(paths: string[], expiresIn: number, options?: CreateSignedUrlOptions): Promise<StorageResult<Array<{
        signedUrl: string;
        token: string;
        path: string;
        error: string | null;
    }>>>;
    createSignedUploadUrl(path: string): Promise<StorageResult<{
        signedUrl: string;
        token: string;
        path: string;
    }>>;
