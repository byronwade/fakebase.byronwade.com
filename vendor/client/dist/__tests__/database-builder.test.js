...
function builder() {
    ...
    return {
        kernel,
        from: (table) => new DatabaseBuilder(kernel, table, "public"),
    };
}
// Seed helper — inserts rows directly and returns the kernel for reuse
async function seedKernel(rows) {
    const kernel = createMemoryKernel(schema);
    await kernel.query({
        schema: "public",
        table: "users",
...
describe("UPSERT", () => {
    ...
    it("updates on conflict (matched by onConflict column)", async () => {
        ...
        await from("users").upsert({ id: "fixed-id-1", name: "Alice Updated", age: 31 }, { onConflict: "id" });
        const { data } = await from("users").select();
        expect(data).toHaveLength(1);
...
//# sourceMappingURL=database-builder.test.js.map