...
function schema(tables, enums = []) {
    return { tables, enums, functions: [], version: 1 };
}
const ID = { name: "id", type: "uuid", nullable: false, primaryKey: true };
...
describe("generateRows — overrides", () => {
    it("applies a per-column override (table.col and schema.table.col forms)", () => {
        ...
        for (const r of rows["public.posts"])
            expect(r.title).toBe("FIXED");
        const rows2 = generateRows(s, {
            rowsPerTable: 4,
            overrides: { "public.posts.title": () => "QUALIFIED" },
        });
        for (const r of rows2["public.posts"])
            expect(r.title).toBe("QUALIFIED");
    });
});
