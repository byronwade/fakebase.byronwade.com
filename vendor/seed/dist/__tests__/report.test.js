...
const find = (report, table, column) => report.find((r) => r.table === table && r.column === column);
describe("describeResolution", () => {
    it("labels primary keys", () => {
        expect(find(describeResolution(SCHEMA), "users", "id").strategy).toBe("primary-key");
    });
    it("labels skipped (defaulted/generated) columns", () => {
        expect(find(describeResolution(SCHEMA), "users", "created_at").strategy).toBe("skipped");
    });
    it("labels foreign keys", () => {
        expect(find(describeResolution(SCHEMA), "posts", "author_id").strategy).toBe("foreign-key");
    });
    it("labels enums", () => {
        expect(find(describeResolution(SCHEMA), "posts", "status").strategy).toBe("enum");
    });
    it("labels semantic name matches", () => {
        expect(find(describeResolution(SCHEMA), "users", "email").strategy).toBe("semantic");
    });
    it("labels raw type fallbacks (no semantic match)", () => {
        expect(find(describeResolution(SCHEMA), "users", "blob").strategy).toBe("type");
    });
