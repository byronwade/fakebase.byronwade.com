...
export function createBuiltinProvider() {
    ...
    const slug = (n) => Array.from({ length: n }, () => pick(WORDS)).join("-");
    ...
    const email = () => `${pick(FIRST_NAMES).toLowerCase()}.${pick(LAST_NAMES).toLowerCase()}${int(1, 99)}@${pick(DOMAINS)}`;
     ...
     */
    function forName(name, type) {
        const n = name.toLowerCase();
        const numericType = type === "numeric" || type === "float4" || type === "float8";
        if (/(^|_)email$/.test(n) || n === "email")
            return () => email();
        if (/(first_?name|given_?name)/.test(n))
        ...
        if (/slug/.test(n))
            return () => slug(int(2, 4));
        if (/title|headline|subject/.test(n))
            return () => titleCase(words(int(2, 5)));
        ...
        if (/(^|_)color$/.test(n) || /(^|_)colour$/.test(n))
            return () => pick(COLORS);
