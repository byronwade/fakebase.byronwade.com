...
import { BookOpen, Box, CornerDownLeft, GitFork, Hash, Moon, Search, Sun } from "lucide-react";
...
import { isActive, navItems, type NavItem } from "./nav-config";
...
const KIND_ICON: Record<SearchKind, typeof Hash> = {
  Page: Hash,
  Doc: BookOpen,
  Example: Box,
};
...
function NavDockItem({ item, pathname }: { item: NavItem; pathname: string }) {
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        render={
          <Link
            ...
            className={cn(
              "relative flex h-8 items-center gap-1.5 rounded-full px-2.5 text-[13px] font-medium whitespace-nowrap outline-none transition-colors focus-visible:ring-2 focus-visible:ring-white/30 sm:px-3",
      ...
      <TooltipContent sideOffset={10} className="xl:hidden">
        {item.label}
      </TooltipContent>
    </Tooltip>
  );
 ...
 *
 * A fixed SLOT reserves the collapsed footprint while an absolutely-positioned,
 * `overflow-hidden` container animates its inline width/height/border-radius as
 * the compact view cross-fades out and the panel cross-fades in. A ResizeObserver
 * follows the panel's live height as results filter; `prefers-reduced-motion`
...
export function NavDock() {
  ...
  // Group in SEARCH_GROUPS order; flat order must match render order for ↑↓.
  const groups = React.useMemo(
    () =>
      SEARCH_GROUPS.map((g) => ({
        ...g,
        items: results.filter((e) => e.kind === g.kind),
  ...
  // Clamp the active index to the current results during render.
            ...
            >
              ...
              <div className="flex items-center gap-2.5 px-3.5 py-3">
                ...
                <input
                  ...
                  placeholder="Search pages, docs, examples…"
                  aria-label="Search"
